...
- Read `docs/deployment.md` before changing Railway resources, buckets,
  migrations, cron jobs, or deployment behavior.

## Repository Shape
- `apps/web/src/app/`: Next.js App Router routes and route tests.
- `apps/web/src/components/`: shared React components.
- `apps/web/src/db/`: read-only PostgreSQL clients and public content queries.
- `apps/web/src/media/`: read-only Railway Bucket media access.
...
- `packages/database/drizzle/postgres/`: generated PostgreSQL migrations and
  metadata; historical D1 migrations remain under `packages/database/drizzle/`.
...
- Use `.ts` and `.tsx`. Plain JavaScript is not allowed unless a tool
  configuration has no TypeScript form; those narrow exceptions are documented
  in `.gitignore`.
- Pin dependency versions exactly. Use `pnpm add` rather than hand-writing a
  version range; `.npmrc` configures pnpm to save exact versions.
...
- Keep Notion off the visitor request path: synced content belongs in PostgreSQL
  and synced media belongs in the private Bucket.
...
- Before removing a script, environment variable, dependency, or configuration,
  search all repository references first, including hidden Git hooks, CI, and
  deployment configuration.
- Do not expose values from `.env`, `.cf-deploy.env`, Railway variables, or
  secrets in logs or responses.
- Do not deploy, mutate remote PostgreSQL, change Railway variables, or modify
  production resources without explicit user authorization.
...
- PostgreSQL schema changes: `pnpm db:migrate` plus targeted schema/query tests

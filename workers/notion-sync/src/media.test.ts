...
const bucketSend = vi.fn().mockResolvedValue(undefined)
const runtime = {
  bucket: { send: bucketSend },
...
describe("rehostImage", () => {
  ...
  it("returns the existing media id without downloading when sourceKey already exists", async () => {
    ...
    expect(bucketSend).not.toHaveBeenCalled()
  })
  ...
  it("downloads, validates, uploads, and inserts a new asset", async () => {
    ...
    expect(bucketSend).toHaveBeenCalledWith(
      expect.objectContaining({
        input: expect.objectContaining({
          Bucket: "media-bucket",
          ContentType: "image/png",
          Key: "media/notion/source-key"
        })
      })
    )
  ...
  it("uses the asset inserted by another isolate when its insert loses the race", async () => {
    ...
    expect(bucketSend).toHaveBeenCalledWith(
      expect.objectContaining({
        input: expect.objectContaining({ Key: "media/notion/race-source-key" })
      })
    )
  ...
  it("shares an in-flight rehost for the same source key", async () => {
    ...
    expect(bucketSend).toHaveBeenCalledTimes(1)
    expect(insert).toHaveBeenCalledTimes(1)
  })
